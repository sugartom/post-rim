...
def main(_):
  ...
  thread_num_array = [1, 8, 64]
  batch_size_2d_array = [[1, 8, 64, 512],
                          [1, 8, 64],
                          [1, 8]]
  ...
  for i in range(len(thread_num_array)):
    ...
    for batch_size in batch_size_arrary:
      ...
      for i in range(warmup_num):
        start = time.time()
        do_inference(FLAGS.server, FLAGS.work_dir, batch_size, 1, 0)
        end = time.time()
        duration = end - start
      ...
      for i in range(thread_num):
        t = threading.Thread(target = do_inference, args = (FLAGS.server, FLAGS.work_dir, batch_size, run_num, i))
